...
async function installConsoleInterceptor(
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      profile: Type.Optional(
        Type.Boolean({
          description:
            "Copy your default Brave profile (cookies, logins, extensions) into the isolated scraping profile. Defaults to false.",
        }),
      ),
    }),
    ...
    async execute(_toolCallId, params) {
      ...
      if (params.profile) {
        try {
          ...
          throw new Error(
            "Failed to sync Brave profile. Ensure Brave has been launched at least once.",
          );
        }
      }
  ...
  pi.registerTool({
    ...
    renderCall(args, theme) {
      const url =
        args.url.length > 60 ? `${args.url.slice(0, 57)}...` : args.url;
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      code: Type.String({
        description:
          "JavaScript code to execute. Access the page DOM and globals.",
      }),
    }),
    renderCall(args, theme) {
      const code =
        args.code.length > 50 ? `${args.code.slice(0, 47)}...` : args.code;
    ...
    renderResult(result, { expanded }, theme) {
      ...
      const preview =
        txt.length > 120 && !expanded ? `${txt.slice(0, 117)}...` : txt;
  ...
  pi.registerTool({
    ...
    renderCall(_args, theme) {
      return new Text(
        theme.fg("toolTitle", theme.bold("browser_content")),
        0,
        0,
      );
    },
  ...
  pi.registerTool({
    ...
    renderCall(_args, theme) {
      return new Text(
        theme.fg("toolTitle", theme.bold("browser_cookies")),
        0,
        0,
      );
    },
  ...
  pi.registerTool({
    ...
    renderCall(args, theme) {
      const msg =
        args.message.length > 40
          ? `${args.message.slice(0, 37)}...`
          : args.message;
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      max: Type.Optional(
        Type.Number({
          description: "Maximum number of logs to return. Defaults to 100.",
        }),
      ),
    }),
    renderCall(args, theme) {
    ...
    async execute(_toolCallId, params) {
      ...
      try {
        ...
        const logs = await page.evaluate(
          (opts: { level?: string; clear?: boolean; max?: number }) => {
            ...
            if (opts.level && opts.level !== "all") {
              filtered = filtered.filter((e: any) => e.level === opts.level);
            }
