...
- Enable profiles independently with `profiles.test.go.mode`, `profiles.test.vitest.mode`, or `profiles.structured.json.mode`; profile apply requires the extension mode and that profile mode to both be `apply`.
