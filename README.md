...
- Enable profiles independently with `profiles.test.go.mode` or `profiles.test.vitest.mode`; profile apply requires the extension mode and that profile mode to both be `apply`.
...
Use `/tool-output` for session savings, profile candidates/bypasses, and storage status. `/tool-output prune` removes expired output; `/tool-output vacuum` compacts the SQLite database. Profile compactness is compared against Pi's visible result (including a truncated tail), not against the recovered raw stream, so it never expands model context. Raw retrieval remains session-scoped through `retrieve_tool_output`.
